...
def train(train_cfg, vlm_cfg):
    ...
    print(
        f"Validation summary: {len(val_loader.dataset)} samples, {len(val_loader)} batches/epoch, batch size {train_cfg.batch_size}"
    )
        ...
        for batch in train_loader:
            ...
            if train_cfg.eval_in_epochs and global_step % 250 == 0:
                ...
                if epoch_accuracy > best_accuracy:
                    ...
                    print(
                        f"Step: {global_step}, Loss: {batch_loss:.4f}, Tokens/s: {tokens_per_second:.2f}, Val Loss: {avg_val_loss:.4f}, Accuracy: {epoch_accuracy:.4f} | Saving checkpoint to {vlm_cfg.vlm_checkpoint_path}"
                    )
                else:
                    print(
                        f"Step: {global_step}, Loss: {batch_loss:.4f}, Tokens/s: {tokens_per_second:.2f}, Val Loss: {avg_val_loss:.4f}, Accuracy: {epoch_accuracy:.4f}"
                    )
                if train_cfg.log_wandb:
                    run.log({"accuracy": epoch_accuracy, "val_loss": avg_val_loss}, step=global_step)

            if train_cfg.log_wandb:
                run.log({"batch_loss": batch_loss, "tokens_per_second": tokens_per_second}, step=global_step)
